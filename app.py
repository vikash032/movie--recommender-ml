...
def initialize_session_state():
    ...
    if "preferences_expanded" not in st.session_state:
        st.session_state.preferences_expanded = False
    if "dl_recs" not in st.session_state:
        st.session_state.dl_recs = None
...
def render_analytics_tab():
    ...
    with tab3:
        ...
        if 'overview' in movies_df.columns:
            text = " ".join(movies_df['overview'].dropna().ast(str))
